		...
		else
		{
			...
			case Commands::PASSWORD: {
				...
				if (existingPlayer)
				{
					...
					auto stmt = conn->createStatement();
					auto res = stmt->executeQuery("SELECT 1 FROM users WHERE Username=" + player.username + " AND Password=" + player.username);
					//Si coincide
					if (res->next())
					{
			...
			case Commands::CHARACTER_NAME:
			{
				...
				//Añadir el personaje a la base de datos
				auto stmt = conn->createStatement();
			...
			case Commands::ASK_PICK_UP:
			{
				...
				if (MapRooms[currentRoom].hasObject)
				{
					...
					auto stmt = conn->createStatement();
					stmt->executeUpdate("INSERT INTO inventarios (Username_FK, ObjectType, ObjectoName, Power) VALUES ("+ player.username +","+ std::to_string((int)(MapRooms[currentRoom].objectToPickup.type)) +","+ MapRooms[currentRoom].objectToPickup.name +","+ std::to_string(MapRooms[currentRoom].objectToPickup.power) +")");
